...
object DruidTransactor {
  ...
  private def createDataSourceResource[M[_]: Sync](factory: => DruidDataSource): Resource[M, DruidDataSource] = {
  ...
  def initial[M[_]: Async: ContextShift](
    ...
    createDataSourceResource(new DruidDataSource).map(Transactor.fromDataSource[M](_, connectEC, blocker))
  }
