...
trait ZioSupport extends Runtime[Unit] { self =>
  ...
  implicit def errorMarshaller[E: ErrorMapper]: Marshaller[E, HttpResponse] =
    Marshaller { implicit ec => a =>
      PredefinedToResponseMarshallers.fromResponse(
        implicitly[ErrorMapper[E]].toHttpResponse(a)
      )
    }
  ...
  implicit def zioRoute[E: ErrorMapper](z: ZIO[Any, E, Route]): Route =
    ctx => {
      ...
      val f = z.fold(
        e =>
          (ctx: RequestContext) =>
            Future.successful(
              Complete(implicitly[ErrorMapper[E]].toHttpResponse(e))
            ),
        a => a
      )
