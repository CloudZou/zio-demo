...
object ApplicationService {
  ...
  def addAsset(
    name: String,
    price: BigDecimal
  ): ZIO[AssetRepository, DomainError, AssetId] =
  ...
  def updateAsset(
    ...
    name: String,
    price: BigDecimal
  ): ZIO[AssetRepository, DomainError, Unit] =
    for {
      _ <- ZIO.accessM[AssetRepository](
             _.assetRepository.update(assetId, name, price)
           )
    } yield ()
  ...
  def getPortfolio(portfolioId: PortfolioId): ZIO[
  ...
  def updatePortfolio(
    ...
    for {
      _ <- ZIO.accessM[PortfolioAssetRepository](
             _.portfolioAssetRepository.add(portfolioId, assetId, amount)
           )
