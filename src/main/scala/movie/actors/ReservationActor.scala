...
class ReservationActor extends Actor {
  ...
  private val demoStorage: mutable.Map[ReservationRequest, ReservationStatus] = mutable.Map.empty

  override def receive: Receive = {
    case GetReservation(request: ReservationRequest) => sender ! demoStorage.get(request)
    case PutReservation(request: ReservationRequest, status: ReservationStatus) => sender ! {
      demoStorage.get(request) match {
        case Some(oldStatus) =>
          ...
          true
        case None =>
          if (request.imdbId == "tt0111161" && request.screenId == "screen_123456") {
    ...
    case ReserveSeat(request: ReservationRequest) => sender ! {
      demoStorage.get(request) match {
        case Some(status) =>
