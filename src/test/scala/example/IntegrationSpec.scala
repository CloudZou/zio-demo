...
import example.infrastructure.{SlickAssetRepository, SlickPortfolioAssetRepository, SlickProductRepository}
...
class IntegrationSpec extends AnyFlatSpec with Matchers with Runtime[Unit] with ScalatestRouteTest with JsonSupport {
  ...
  class TestEnv extends SlickAssetRepository with SlickPortfolioAssetRepository with SlickProductRepository with TestDatabaseProvider
